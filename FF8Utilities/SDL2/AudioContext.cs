...
namespace FlowTimer {
    ...
    public class AudioContext {
        ...
        public void QueueAudio(byte[] pcm) {
            ...
            IntPtr ptr = Marshal.AllocHGlobal(pcm.Length);
            try
            {
                Marshal.Copy(pcm, 0, ptr, pcm.Length);
                SDL_QueueAudio(DeviceId, ptr, pcm.Length);
            }
            finally
            {
    ...
    public static class Wave {
        ...
        public static bool LoadWAV(byte[] bytes, out byte[] pcm, out SDL_AudioSpec spec) {
            pcm = new byte[0];
